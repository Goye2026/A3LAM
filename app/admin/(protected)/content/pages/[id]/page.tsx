...
export default async function EditCmsPage({ params }: Props) {
  ...
  return <div className="admin-route"><header className="admin-route-heading"><div><p className="eyebrow">{copy.adminCmsPages}</p><h1>{result.record.title}</h1></div></header><CmsEditorialEditor kind="page" initialRecord={result.record} copy={copy} capabilities={capabilities} recoveryScope={access.principal?.id ?? "unknown"} /></div>;
}
