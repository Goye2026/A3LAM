...
export default async function AdminPostsPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return <div className="admin-route"><header className="admin-route-heading"><div><p className="eyebrow">{copy.adminContent}</p><h1>{copy.adminCmsPosts}</h1><p className="route-description">{copy.adminCmsEditor}</p></div></header><CmsEditorialList kind="post" data={result.data} copy={copy} unavailable={result.unavailable} query={query} status={status} capabilities={capabilities} /></div>;
}
