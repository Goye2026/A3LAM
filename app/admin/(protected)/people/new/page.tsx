...
export default async function NewPersonPage() {
  ...
  return <div className="admin-route"><header className="admin-route-heading"><div><Link className="admin-back-link" href="/admin/people">{copy.adminPeople}</Link><p className="eyebrow">{copy.adminTitle}</p><h1>{copy.adminPersonNew}</h1></div></header>{unavailable ? <p className="admin-alert" role="alert">{copy.adminDatabaseError}</p> : <AdminPersonForm copy={copy} categories={categories} mediaStatus={getStorageStatus()} mediaProviderState={getStorageProviderState()} currentMedia={null} />}</div>;
}
