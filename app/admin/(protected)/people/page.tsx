...
export default async function AdminPeoplePage({ searchParams }: PageProps) {
  ...
  try {
    data = await adminRepository.listPeople({ query, status, page });
  } catch {
    unavailable = true;
  }
  const hasNext = data ? data.page * data.pageSize < data.total : false;
  const previousHref = `/admin/people?${new URLSearchParams({ ...(query ? { q: query } : {}), ...(status ? { status } : {}), page: String(Math.max(page - 1, 1)) })}`;
  const nextHref = `/admin/people?${new URLSearchParams({ ...(query ? { q: query } : {}), ...(status ? { status } : {}), page: String(page + 1) })}`;
  ...
  return (
    <div className="admin-route">
      ...
      <form className="admin-filter-form" method="get">
        ...
        <select id="admin-status" className="admin-input" name="status" defaultValue={status}>
          <option value="">{copy.adminAllStatuses}</option>
          <option value="draft">{copy.adminDraft}</option>
          <option value="review">{copy.adminReview}</option>
          <option value="published">{copy.adminPublished}</option>
          <option value="archived">{copy.adminArchived}</option>
        </select>
      ...
      <nav className="admin-pagination" aria-label={copy.adminPeople}>
        {page > 1 ? <Link className="button button-quiet" href={previousHref}>{copy.adminPagePrevious}</Link> : <span />}
        <span>{page} / {data ? Math.max(Math.ceil(data.total / data.pageSize), 1) : 1}</span>
        {hasNext ? <Link className="button button-quiet" href={nextHref}>{copy.adminPageNext}</Link> : <span />}
      </nav>
    </div>
  );
}
