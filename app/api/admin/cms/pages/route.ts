...
import { parseCmsEditorialMutation } from "@/lib/cms/editorialValidation";
...
export async function GET(request: Request) {
  ...
  try {
    return NextResponse.json(await editorialRepository.list("page", listOptions(request)), { headers: { "Cache-Control": "private, no-store" } });
  } catch (error) {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    parseCmsEditorialMutation(body, "page");
    const record = await editorialRepository.create("page", body, access.principal?.id ?? null);
