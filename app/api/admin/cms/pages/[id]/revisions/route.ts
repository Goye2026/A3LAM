...
export async function GET(request: Request, { params }: Props) {
  ...
  try {
    ...
    return NextResponse.json({ items: await editorialRepository.listRevisions("page", id) }, { headers: { "Cache-Control": "private, no-store" } });
...
export async function POST(request: Request, { params }: Props) {
  ...
  try {
    ...
    const record = await editorialRepository.restoreRevision("page", id, input.revisionId, input.expectedVersion, gate.principal.id);
