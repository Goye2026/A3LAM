...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const record = await editorialRepository.get("page", id);
...
export async function PUT(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const { id } = await context.params;
    const record = await editorialRepository.update("page", id, await readBoundedJson(request), access.principal?.id ?? null);
    if (!record) return NextResponse.json({ error: "NOT_FOUND", message: "Content not found" }, { status: 404 });
