...
export function A3lamDocumentUploader({ copy, disabled, state = "IDLE", progress, failureMessage, onFileSelected, onRetry }: Props) {
  ...
  return (
    <section className="admin-panel ai-uploader" aria-labelledby="ai-uploader-title">
      ...
      <div className="ai-uploader-dropzone" aria-disabled={disabled} onDragOver={(event) => { if (!disabled) event.preventDefault(); }} onDrop={(event) => { event.preventDefault(); if (!disabled) inspect(event.dataTransfer.files?.[0]); }}>
