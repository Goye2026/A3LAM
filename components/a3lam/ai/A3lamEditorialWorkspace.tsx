...
export function A3lamEditorialWorkspace({ copy }: { copy: Copy }) {
  ...
  const hasUnresolvedFacts = facts.some((fact) => fact.status === "UNREVIEWED");
  const reviewedClaims = claims.filter((claim) => claim.status === "VERIFIED" || claim.status === "REJECTED");
  const qualityState = !result ? "BLOCKED" : hasUnresolvedFacts || claims.some((claim) => claim.status === "CONFLICTED" || claim.status === "NEEDS_VERIFICATION") ? "WARNING" : "PASS";
  ...
  return (
    <section className="ai-editorial-workspace" aria-labelledby="ai-editorial-workspace-title">
      ...
      {activeStep === 0 ? <section className="ai-step-panel" aria-labelledby="ai-step-document-title">
        ...
        <A3lamDocumentUploader copy={copy} disabled={false} localOnly onFileSelected={() => { setNotice(copy.adminAiLocalOnly); setError(null); }} />
      </section> : null}
      ...
      {activeStep === 2 ? <section className="ai-step-panel" aria-labelledby="ai-step-facts-title">
        ...
        <div className="ai-fact-list">{facts.map((fact) => <article className={`ai-fact-card ai-fact-card-${fact.status.toLowerCase()}`} key={fact.id}>
          <header><div><span className="ai-fact-field">{fact.fieldPath}</span><h4>{fact.value}</h4></div><span className="ai-status-pill">{factStatusLabel(fact.status, copy)}</span></header>
      ...
      {activeStep === 3 ? <section className="ai-step-panel" aria-labelledby="ai-step-generation-title">
        ...
        <div className="ai-generation-config"><label>{copy.adminAiGenerationChooseMode}<select value={mode} onChange={(event) => setMode(event.target.value as AiGenerationMode)}>{modes.map((item) => <option value={item} key={item}>{modeLabel(item, copy)}</option>)}</select><small>{modeDescription(mode, copy)}</small></label><label>{copy.adminAiGenerationChooseLanguage}<select value={language} onChange={(event) => setLanguage(event.target.value as AiGenerationLanguage)}>{languages.map((item) => <option value={item} key={item}>{languageLabel(item, copy)}</option>)}</select></label></div>
      ...
      {activeStep === 4 ? <section className="ai-step-panel" aria-labelledby="ai-step-draft-title">
        ...
        {result?.draft ? <div className="ai-draft-preview"><div className="ai-draft-heading"><div className="ai-draft-avatar">A</div><div><span>{modeLabel(mode, copy)}</span><h4>{valueOf(result.draft.identity.nativeName?.value) || demoExtraction.fileName}</h4><p>{languageLabel(language, copy)} · {copy.adminAiAiWording}</p></div></div><div className="ai-draft-sections">{result.claims.map((claim) => <article key={claim.id}><div><span>{claim.fieldPath}</span><strong>{valueOf(claim.value)}</strong></div><span className="ai-status-pill">{claimStatusLabel(claim, copy)}</span></article>)}</div></div> : <p className="admin-empty">{copy.adminAiNoLocalDraft}</p>}
      ...
      {activeStep === 6 ? <section className="ai-step-panel" aria-labelledby="ai-step-review-title">
        ...
        <p className="ai-private-draft-banner" role="status">{copy.adminAiPrivateDraftNotice}</p>
        <button type="button" className="button button-quiet" onClick={() => setActiveStep(5)}>{copy.adminAiBack}</button>
      </section> : null}
    </section>
  );
}
